resource "null_resource" "DeployDev" {
  ...
  provisioner "local-exec" {
    command = "docker run --rm -d --env-file ${var.envFileDB} --hostname ${var.DBHost} --name ${var.DBHost} -e POSTGRES_PASSWORD=${var.POSTGRES_PASSWORD} --network localNetwork -p ${var.DBPort}:${var.DBContainerPort} ${var.imageDB}"
  }
  provisioner "local-exec" {
